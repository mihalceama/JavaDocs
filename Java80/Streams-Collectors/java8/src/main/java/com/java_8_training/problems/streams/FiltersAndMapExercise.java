...
public class FiltersAndMapExercise {
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter( value -> value==name ).collect(toList());
        return personName;
    }
    ...
    public static List<Integer> filterNull(){
        ...
        evenNumbers = input.filter(number -> number != null).collect(toList());
        return evenNumbers;
    }
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age){
       ...
       persons = personList.stream().filter(person -> person.getName() == name && person.getAge() == age).collect(toList());
    ...
    public static List<String> getOnlyNames(List<Person> personList){
        ...
        //TODO #6
        // HINT: use map
    collect = personList.stream().map( name -> name.getName()).collect(toList());
        return  collect;
    }
    ...
    public static boolean areAllPersonsBelowAge(List<Person> personList, Integer age){
        ...
        overAge = personList.stream().allMatch(personAge -> personAge.getAge() < 70);
        return overAge;
    }


    public static int findTheMinimumNumber(){
        Stream<Integer> input =  Stream.of(5, 2, 200, 33, 150, 100);
        //TODO #11
        // HINT: use reduce
        int min =0;
        int max = 1000;

        min = input.reduce(max, (acc, x) -> Integer.min(acc, x));
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        min = personList.stream().map( x -> x.getAge() ).reduce(100,Integer::min);
    ...
    public static double findAgeAverage(List<Person> personList){
        double sum = 0;
        //TODO #13
        sum = personList.stream().map(average -> average.getAge()).reduce(0,Integer::sum);
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList){
        ...
        persons = personList.stream().filter(firstName ->firstName.getName().startsWith("J")).map(nationality -> nationality.getNationality()).distinct().collect(toList());
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        List<Integer> someNumbers = Arrays.asList(4, 36, 9, 81);
    ...
    public static int sumStringLength(){
        ...
        sum = strings.stream().map(word -> word.length()).reduce(0, Integer::sum);
        return sum;
    }
    ...
    public static  int sumSalary(List<Person> personList){
        int sum =0;
        //TODO #17
        sum = personList.stream().map(value -> value.getSalary()).reduce(0,(acc, value)-> acc + value);
        return sum;
    }
