...
public class FiltersAndMapExercise {
    ...
    public static  List<Integer> findEvenNumbers() {
        ...
        evenNumbers = input.filter((Integer i) ->i%2 == 0).collect(toList());
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter((String a)-> a == name).collect(toList());
    ...
    public static List<Integer> filterNull(){
        ...
        evenNumbers = input.filter((Integer i) -> i != null).collect(toList());
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age){
        ...
        persons = personList.stream().filter((Person p)->p.getName() == name && p.getAge() == age).collect(toList());
    ...
    public static List<Integer> doubleNumbers() {
        ...
        input.map((Integer i) -> i * 2).collect(toList());
    ...
    public static List<String> getOnlyNames(List<Person> personList){
        ...
        collect = personList.stream().map((Person person) -> person.getName()).collect(toList());
    ...
    public static Optional<Person> findFirstAllMalePersons(List<Person> personList){
        ...
        personOptional = personList.stream().filter(Person::isMale).findAny();
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList){
        ...
        persons = personList.stream().filter(person -> person.getName().startsWith("J")).distinct().map(person -> person.getNationality()).collect(toList());
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        firstSquareDivisibleByThree = someNumbers.stream().map(x ->(x * x)).filter(x -> (x % 3 == 0)).findFirst();
    ...
    public static int sumStringLength(){
        ...
        sum = strings.stream().map(a -> a.length()).reduce(0,(acc, x) -> (acc + x));
